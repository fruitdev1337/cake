...
// find comment // CUSTOM to see custom fixes
(function() {
	'use strict';
	...
	BX.Sale.OrderAjaxComponent = {
		...
		showWarnings: function()
		{
			...
			if (false && currentDelivery && currentDelivery.CALCULATE_ERRORS) // убираю поле с ненужной ошибкой
			{
		...
		clickOrderSaveAction: function(event)
		{
			...
			if (this.isValidForm() && this.checkAgreement())
			{
				this.allowOrderSave();
		...
		createBasketItemContent: function(data)
		{
			...
			// !----- custom fields
			let customNodes = [];
			let custom_add = BX.getCookie('custom_add');
			if(custom_add){
				custom_add = JSON.parse(custom_add);
				if(custom_add[data.ID]){
					let custom_data = custom_add[data.ID];
					if(custom_data.text){
						customNodes.push(
							BX.create('DIV', {
								props: {className: 'custom-add-title'},
								text: 'Текст на изделии:',
							})
						);
						customNodes.push(
							BX.create('DIV', {
								props: {className: 'custom-add-value'},
								text: custom_data.text,
							})
						);
					}
					if(custom_data.imgs){
						let img_src = custom_data.imgs[0].src;
						let img_name = custom_data.imgs[0].file_name;
						customNodes.push(
							BX.create('DIV', {
								props: {className: 'custom-add-title'},
								text: 'Изображение:',
						...
						customNodes.push(
							BX.create('DIV', {
								props: {className: 'custom-add-value img_link'},
								html: '<a href="'+img_src+'" target="_blank">'+img_name+'</a>',
							})
						);
					}
				}
			}
			let custom_div = false;
			if(customNodes.length){
				custom_div = BX.create('DIV', {props: {className: 'bx-custom-container 123'}, children: customNodes})
			}
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				let hide = '';
				if(property.getId() == 20 || property.getId() == 22 || property.getId() == 24 ){ // cake text and added img
					hide = ' hide';
				}

				BX.addClass(propsItemNode, "form-group bx-soa-customer-field"+hide);
				...
				if(property.getId() == 21){
					propsItemNode.setAttribute('data-date', property.getValue());
				}
				if(property.getId() == 23){
					let hour = property.getSettings().min_hour;
					propsItemNode.setAttribute('data-hour', hour);
				}
				propsItemNode.appendChild(label);
			}
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
                ...
                if (settings.IS_PHONE == 'Y') {
                    inputs[i].placeholder = '+7 (999) 999 99 99';
                    BX.addClass(inputs[i], 'form-control bx-soa-customer-input bx-ios-fix masked');
                }
		...
		alterDateProperty: function(settings, inputText)
		{
			...
			BX.bind(appendNode, 'click', BX.delegate(function(e){
				...
				changeCalendar(); // main/script.js убираем выбор прошедших дат
			}, this));
		},
		...
		{
			let curr_template = '<i class="fa fa-rub"><span>₽</span></i>';
			...
			else
			{
				...
				{
					if(this.result.BRANDS_COUNT > 1) {
						let price_calc = total.DELIVERY_PRICE * this.result.BRANDS_COUNT;
						deliveryValue = price_calc + ' '+curr_template;
					}else{
			...
			{
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params)); // CUSTOM
			}
			...
			else
			{
				...
				if(this.result.BRANDS_COUNT){
					let t_price = parseFloat(total.ORDER_PRICE) + parseFloat(total.DELIVERY_PRICE * this.result.BRANDS_COUNT);
					t_price = t_price + ' ' + curr_template;
					this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_IT'), t_price, {total: true}));
				}else{
		...
		removeAddressRow: function(){ // custom function. Remove adress input if pickup delivery.
			let delivery_id = document.querySelector('input[name="DELIVERY_ID"]:checked').value;
			if(delivery_id == 3) { // самовывоз
				let date_row = document.querySelector('[data-property-id-row="21"]');
				let address_row = document.querySelector('[data-property-id-row="7"]');
				if(date_row){
					// date_row.classList.add('hidden');
				}
				if(address_row){
					address_row.classList.add('hidden');
				}
			}
		},
		checkAgreement(){
			let checked = $('#agreement-box').prop('checked');

			if(!checked){
				$('#agreement-box').parent().addClass('check-error');
			}

			return checked;
		},
		doGaEvent(){
			// console.log('zakaz poshel');
