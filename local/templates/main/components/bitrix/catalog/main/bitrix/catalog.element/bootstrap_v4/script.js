(function(window){
	...
	window.JCCatalogElement = function(arParams)
	{
		...
		this.obReadyTime = null;
		this.obPriceRerKG = null;
	...
	window.JCCatalogElement.prototype = {
		...
		init: function()
		{
			...
			if (this.config.showSkuProps)
			{
				...
				this.obReadyTime = BX(this.visual.DISPLAY_READY_TIME_DIV);
			}
			
			this.obPriceRerKG = BX(this.visual.DISPLAY_PRICE_PER_KG_DIV);
		...
		changeInfo: function()
		{
			...
			if (index > -1)
			{
				...
				if (this.config.showSkuProps)
				{
					...
					if (this.obReadyTime)
					{
						if (!this.offers[index].DISPLAY_READY_TIME_HTML)
						{
							BX.adjust(this.obReadyTime, {style: {display: 'none'}, html: ''});
						}
						else
						{
							BX.adjust(this.obReadyTime, {style: {display: ''}, html: this.offers[index].DISPLAY_READY_TIME_HTML});
						}						
					}
					
					if (this.obPriceRerKG)
					{
						if (!this.offers[index].DISPLAY_PRICE_PER_KG_HTML)
						{
							BX.adjust(this.obPriceRerKG, {style: {display: 'none'}, html: ''});
						}
						else
						{
							BX.adjust(this.obPriceRerKG, {style: {display: ''}, html: this.offers[index].DISPLAY_PRICE_PER_KG_HTML});
						}						
					}
				}
