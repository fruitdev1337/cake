...
<div class="title_box">
    <h2><?=Loc::getMessage('K_TITLE_IN_SALE',['NAME' => $arResult["NAME"]])?></h2>
</div>
            <?php
            ...
            $arSectionBrand = [];
            //ссылка фильтр каталога с выбранным брендом
            $strFilter = "";
               $propFilter =  Bitrix\Iblock\PropertyTable::getList([
                        'filter' => ['=CODE' => $PROP_BRAND,"IBLOCK_ID"=>IntVal($arParams['CATALOG_IBLOCK_ID'])],
                        'select' => ['ID']
               ])->fetch();

               if($propFilter)
               {
                   $strFilter = "?arFilterCatalog_{$propFilter["ID"]}_".abs(crc32($ElementID))."=Y&set_filter=";
               }

           while($arFields = $res->GetNext())
            {

                $arFields['SECTION_PAGE_URL'].= $strFilter;
                $arSectionBrand[] = $arFields;
            }
            //если есть разделы то выводм товары согласно разделам
            if($arSectionBrand):?>
                <?foreach ($arSectionBrand as $section):?>
