...
else
{
    ...
    <div class="orders-history-link">
        ...
        {
            ?>
            <a class="small" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_VIEW_ORDERS_HISTORY")?></a>
            <?
        }
        if ($_REQUEST["filter_history"] == 'Y')
        {
            ?>
            <a class="small" href="<?=$APPLICATION->GetCurPageParam("", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_CUR_ORDERS")?></a>
            <?
            ...
            {
                ?>
                <a class="small" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y&show_canceled=Y", $clearFromLink, false)?>"><?echo Loc::getMessage("SPOL_TPL_VIEW_ORDERS_CANCELED")?></a>
                <?
            }
        }
        ?>
	...
	if (!count($arResult['ORDERS']))
	{
		if ($_REQUEST["filter_history"] == 'Y')
		{
			...
			{
				?>
				<div class="bx-sbb-empty-cart-text"><?= Loc::getMessage('SPOL_TPL_EMPTY_CANCELED_ORDER')?></div>
				<?
			}
			else
			{
				?>
				<div class="bx-sbb-empty-cart-text"><?= Loc::getMessage('SPOL_TPL_EMPTY_HISTORY_ORDER_LIST')?></div>
				<?
			}
		}
		else
		{
			?>
			<div class="bx-sbb-empty-cart-text"><?= Loc::getMessage('SPOL_TPL_EMPTY_ORDER_LIST')?></div>
			<?
		}
	}
	?>
	...
	if (!count($arResult['ORDERS']))
	{
        ...
        <div class="bx-sbb-empty-cart-btn">
            <a href="<?=htmlspecialcharsbx($arParams['PATH_TO_CATALOG'])?>" class="catalog-link btn-primary"><?=Loc::getMessage('SPOL_TPL_LINK_TO_CATALOG')?></a>
        </div>
		<?
	}
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $arResult['SORT_TYPE'] == 'STATUS')
			{
				...
				?>
				<div class="mb-3 orders-status-title title_box">
			...
			?>
            <div class="sale-order-list-item">
                <div class="mx-0 sale-order-list-title-container">
                    <h3 class="mb-1 mt-1 p-2">
                ...
                <div class="mx-0 mb-5">
                    <div class="sale-order-list-inner-container">
                        ...
                        foreach ($order['SHIPMENT'] as $shipment)
                        {
                            ...
                            ?>
                            <div class="mb-3 sale-order-list-shipment">
                                <div class="col">
                        ...
                        <div class="pb-3 sale-order-list-inner-row">
                            <div class="sale-order-list-link">
                                <div class="col-auto sale-order-list-about-container">
                                    <a class="g-font-size-15 sale-order-link sale-order-list-about-link" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_DETAIL"])?>"><?=Loc::getMessage('SPOL_TPL_MORE_ON_ORDER')?></a>
                                </div>
                                <div class="col-auto sale-order-list-repeat-container">
                                    <a class="g-font-size-15 sale-order-link sale-order-list-repeat-link" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_COPY"])?>"><?=Loc::getMessage('SPOL_TPL_REPEAT_ORDER')?></a>
                                </div>
                                <?
                                if ($order['ORDER']['CAN_CANCEL'] !== 'N')
                                {
                                    ...
                                    <div class="col-auto sale-order-list-cancel-container">
                                        <a class="g-font-size-15 sale-order-link sale-order-list-cancel-link" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_CANCEL"])?>"><?=Loc::getMessage('SPOL_TPL_CANCEL_ORDER')?></a>
                                    </div>
                                    <?
                                }
                                ?>
            ...
            <div class="sale-order-list-item">
                ...
                <div class="mb-5">
                    <div class="col pt-3 sale-order-list-inner-container">
                        <div class="pb-3 sale-order-list-inner-row sale-order-list-link">
