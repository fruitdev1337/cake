...
if (IsModuleInstalled("iblock") && $arParams['IBLOCK_ID'] && IsModuleInstalled("krayt.retail"))
{
	$arFilter = array(
		"ID" => intval($arParams['IBLOCK_ID']),
		"SITE_ID" => SITE_ID,
		"ACTIVE" => "Y"
	);
	...
	{
		if (CModule::IncludeModule("iblock") && CModule::IncludeModule("krayt.retail"))
		{
			...
			{
			    $arSelect  = array("ID", "SECTION_PAGE_URL", "PICTURE", "DESCRIPTION");

			    if($arParams['ICON_MENU'] && (!empty($arParams['ICON_MENU'])))
                {
                    $arSelect[] = $arParams['ICON_MENU'];
                }
                if($arParams['ACTION_MENU'] && (!empty($arParams['ACTION_MENU'])))
                {
                    $arSelect[] = $arParams['ACTION_MENU'];
                    $arUFBanner =  CUserTypeEntity::GetList( array(),
                        array(
                            "LANG" => "ru",
                            "ENTITY_ID" => "IBLOCK_".intval($arParams['IBLOCK_ID'])."_SECTION",
                            "FIELD_NAME" => $arParams['ACTION_MENU']
                        ) )->Fetch();

                }

				$dbSections = CIBlockSection::GetList(array(), array("IBLOCK_ID" => $arIBlock["ID"]), false, $arSelect);
				while($arSections = $dbSections->GetNext())
				{
                    ...
                    if($arParams['ICON_MENU'] && (!empty($arParams['ICON_MENU'])))
                    {
                        if($arSections[$arParams['ICON_MENU']])
                        {
                            $iconSrc = CFile::GetFileArray($arSections[$arParams['ICON_MENU']]);

                            if ($iconSrc)
                                $arResizeIcon = CFile::ResizeImageGet(
                                    $arSections[$arParams['ICON_MENU']],
                                    array("width" => 50, 'height'=>50),
                                    BX_RESIZE_IMAGE_PROPORTIONAL,
                                    true
                                );
                            $arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["ICON"] = $iconSrc ? $arResizeIcon["src"] : false;
                        }
                    }

                    if($arParams['ACTION_MENU'] && (!empty($arParams['ACTION_MENU'])))
                    {
                        if($arSections[$arParams['ACTION_MENU']])
                        {
                            $banner = CKray_retail::getBannerById(
                                isset($arUFBanner['SETTINGS']['IBLOCK_ID'])?$arUFBanner['SETTINGS']['IBLOCK_ID']:0,
                                $arSections[$arParams['ACTION_MENU']]);
                            if($banner)
                            {
                                $arSectionsInfo[crc32($arSections["SECTION_PAGE_URL"])]["BANNER"] = $banner;
                            }
                        }
                    }
...
foreach($arResult as $key=>$arItem)
{
	...
	$arItem["PARAMS"]["icon"] = $arSectionsInfo[$arItem["PARAMS"]["item_id"]]["ICON"];
	$arItem["PARAMS"]["banner"] = $arSectionsInfo[$arItem["PARAMS"]["item_id"]]["BANNER"];
