...
BX.Catalog.SetConstructor = (function()
{
	...
	SetConstructor.prototype.generateSliderStyles = function()
	{
		var styleNode = BX.create("style", {
			html:	".catalog-set-constructor-slider-slide-"+this.jsId+"{width: " + this.numSliderItems*25 + "%;}"+
					...
					".catalog-set-constructor-slider-slide-"+this.jsId+"{width: " + this.numSliderItems*11.1*3 + "%;}}" +
					"@media (max-width:767px){"+
            		".catalog-set-constructor-slider-slide-"+this.jsId+"{width: " + this.numSliderItems*320 + "px;}" +
            		".catalog-set-constructor-slider-item-container-"+this.jsId+"{width: 320px;}}",
			attrs: {
				...
				children: [
					BX.create("div", {
							...
							children: [
								...
								BX.create("div", {
									attrs: {
										className: "catalog-set-constructor-slider-item-price product_item__price"
									},
									children: [
										BX.create("div", {
											attrs: {
												className: "catalog-set-constructor-slider-item-price-new price"
											},
											html: "<span>"+itemPrintPrice+" <i class='fa fa-rub'></i></span>" + " <span class='set-item-quantity'>x " + itemBasketQuantity + itemMeasure + "</span>"
										}),
										BX.create("div", {
											attrs: {
												className: "catalog-set-constructor-slider-item-price-old old_price"
											},
                                            html: itemPrice != itemOldPrice ? "<span>"+itemPrintOldPrice+"</span>" : ""
										})
									]
								}),
								BX.create("div", {
									...
									children: [
										BX.create("a", {
											...
											html: '<span class="fa fa-plus"></span><span>' + this.messages.ADD_BUTTON + '</span>'
										})
									]
								})
							]
					...
					children: [
						...
						BX.create("td", {
							...
							children: [
								BX.create("span", {
									attrs: {
										className: "catalog-set-constructor-product-new-price price"
									},
									html: '<span>' + itemPrintPrice + ' <i class="fa fa-rub"></i></span> <span class="set-item-quantity">x ' + itemBasketQuantity+' '+ itemMeasure + '</span>'
								}),
								BX.create("br"),
								BX.create("span", {
									attrs: {
										className: "catalog-set-constructor-product-old-price old_price"
									},
									html: itemPrice != itemOldPrice ? '<span>' + itemPrintOldPrice + ' <i class="fa fa-rub"></i></span>' : ""
								})
							]
						}),
						BX.create("td", {
	...
	SetConstructor.prototype.recountPrice = function()
	{
		...
		this.setPriceDuplicateCont.innerHTML = '<span>' + BX.Currency.currencyFormat(sumPrice, this.currency, true) + '</span>';
		if (Math.floor(sumDiffDiscountPrice*100) > 0)
		{
			this.setOldPriceCont.innerHTML = '<span>' + BX.Currency.currencyFormat(sumOldPrice, this.currency, true) + '</span>';
			this.setDiffPriceCont.innerHTML = '<span>' + BX.Currency.currencyFormat(sumDiffDiscountPrice, this.currency, true) + '</span>';
			BX.style(this.setOldPriceRow, 'display', 'flex');
			BX.style(this.setDiffPriceRow, 'display', 'flex');
		}
		else
		{
