...
$(function () {
    ...
    slider_set = new Swiper('.catalog-set-constructor-slider-container', {
        slidesPerView: 4,
        navigation: {
...
BX.Catalog.SetConstructor = (function()
{
	...
	SetConstructor.prototype.generateSliderStyles = function()
	{
        ...
        var styleNode = BX.create("style", {
            ...
            ".catalog-set-constructor-slider-slide-"+this.jsId+"{width: " + this.numSliderItems*100 + "%;}" +
            ".catalog-set-constructor-slider-item-container-"+this.jsId+"{width: " + (100/this.numSliderItems) + "%;}}",
	...
	SetConstructor.prototype.deleteFromSet = function()
	{
		...
		if (target && target.hasAttribute('data-role') && target.getAttribute('data-role') == 'set-delete-btn')
		{
			...
			newSliderNode = BX.create("div", {
				attrs: {
					className: " swiper-slide catalog-set-constructor-slider-item-container catalog-set-constructor-slider-item-container-" + this.jsId,
					"data-id": itemId,
								...
								BX.create("div", {
									...
									children: [
										BX.create("div", {
											...
											html: '<strong>' + itemPrintPrice + '  <i class="fa fa-rub"></i></strong> <span>x ' + itemBasketQuantity + ' '+ itemMeasure + '</span>'
										}),
										BX.create("div", {
											attrs: {
												className: "catalog-set-constructor-slider-item-price-old"
											},
											html: itemPrice != itemOldPrice ? '<strong>' + itemPrintOldPrice + '  <i class="fa fa-rub"></i></strong>' : ""
										})
									]
								}),
								BX.create("div", {
            ...
            slider_set.addSlide(1,newSliderNode);
			...
			if (this.numSetItems <= 0 && !!this.emptySetMessage)
				BX.adjust(this.emptySetMessage, { style: { display: 'block' }, html: this.messages.EMPTY_SET });
	...
	SetConstructor.prototype.addToSet = function()
	{
		...
		if (!!target && target.hasAttribute('data-role') && target.getAttribute('data-role') == 'set-add-btn')
		{
            ...
            if ($(window).width() <= 575.98) {
                slider_set.removeSlide(slider_set.activeIndex);
            } else {
                slider_set.removeSlide(item);
            }
