...
if (\Bitrix\Main\Loader::includeModule("krayt.retail") && \Bitrix\Main\Loader::includeModule('currency')) {
    ...
    if ($MIN_PRICE['VALUE']):
        $arParams['MIN_PRICE'] = $MIN_PRICE['VALUE'];
        $arParams['MIN_PRICE_CURRENCY']  = $MIN_PRICE['CURRENCY'];
    endif;
}
use Ttcr\CakeOrder;
$add_img = CakeOrder::checkAddImg($arResult['BASKET_ITEM_RENDER_DATA']);
foreach($arResult['BASKET_ITEM_RENDER_DATA'] as &$basket_item){
    $add_img = CakeOrder::checkAddImg($basket_item['PRODUCT_ID']);
    if($add_img){
        $basket_item['CUSTOM_ADD'] = true;
    }
}
//use Ttcr\Cake;
//Cake::sendData(168);
//dump(CakeOrder::getOrderProps(166));
//dump($arResult['BASKET_ITEM_RENDER_DATA'][0]['CUSTOM_F'] = '123123123');
?>
    ...
    <div class="title_box">
        <div class="row align-items-sm-center">
            ...
            <div class="col-sm-auto col-12">
                <a href="<?=$APPLICATION->GetCurPageParam('clear')?>"
                   class="clear_basket btn-green-border"
                   <?=($arResult['EMPTY_BASKET'] ? 'style="display:none;"' : '');?>><?=Loc::getMessage('SALE_CLEAR_BASKET');?></a>
            </div>
        </div>
    </div>
			...
			<div class="col-12">
				<div class="basket-items-list-wrapper basket-items-list-wrapper-height-fixed basket-items-list-wrapper-light<?=$displayModeClass?>"
					...
					<div class="basket-items-list-container" id="basket-items-list-container">
                        ...
                        <? /* if($add_img):?>
                        <div class="basket-title-desc">Текст надписи на торте:</div>
                        <div class="form  form--cart">
                            <div class="form-group">
                                <input type="text" class="form-control js-cust-field" id="form-title" placeholder="Ваш текст">
                            </div>
                            <div class="form-group">
                                <div class="fileform">
                                    <div id="fileformlabel"></div>
                                    <div class="selectbutton">
                                        <img src="<?=SITE_TEMPLATE_PATH?>/img/paper-clip.svg" alt="">
                                        <span>Прикрепить файл</span>
                                    </div>
                                    <input type="file" name="upload" id="upload-image" class="js-cust-field" onchange="getName(this.value);" />
                                </div>
                            </div>
                        </div>

                            <div class="form-result-desc">
                                <div class="form-result-desc__title"></div>
                                <button class="btn-primary  btn-primary--green  js-button-change">Изменить</button>
                                <button class="btn-primary  btn-primary--red  js-button-delete">Удалить</button>
                            </div>
                        <?endif; */?>
					</div>
				</div>
