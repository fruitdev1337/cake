...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
    ...
    //получение коэфициент единицы измеренеия

    $measureCof = CCatalogMeasureRatio::getList([],[
        '=PRODUCT_ID' => $arResult["ELEMENTS"]
    ]);
    $arMeasureCof = [];
    while ($m = $measureCof->Fetch())
    {
        $arMeasureCof[$m['PRODUCT_ID']] = $m;
    }
    $allmeasure = \CKray_retail::getAllMeasure();
    ...
    while($arElement = $rsElements->Fetch())
    {
        ...
        if($allmeasure[$arElement['CATALOG_MEASURE']])
        {
            $arElement['MEASURE'] = $allmeasure[$arElement['CATALOG_MEASURE']];

        }
        if($arMeasureCof[$arElement['ID']] && $arMeasureCof[$arElement['ID']]['RATIO'] != 1 && $arElement['CATALOG_TYPE'] == 1)
        {
            $catalogPriceValue = '';
            foreach ($arResult["PRICES"] as $key => $value) {
                ...
                if (
                    !$value['CAN_VIEW']
                    || !isset($arElement[$catalogPriceValue])
                    || $arElement[$catalogPriceValue] == ''
                )
                    continue;
            }

            if(!empty($catalogPriceValue))
                $arElement[$catalogPriceValue] = $arMeasureCof[$arElement['ID']]['RATIO']*$arElement[$catalogPriceValue];

            if($arElement['MEASURE'])
                $arElement['MEASURE']['TEXT'] = Loc::getMessage("K_MEASURE_ZA",
                    ['MEASURE'=>$arElement['MEASURE']['SYMBOL_RUS'],'CNT' => $arMeasureCof[$arElement['ID']]['RATIO']]);

        }else{
            $arElement['MEASURE']['TEXT'] = Loc::getMessage("K_MEASURE_ZA",
                ['MEASURE'=>$arElement['MEASURE']['SYMBOL_RUS'],'CNT' => 1]);
        }
...
foreach($arResult["SEARCH"] as $i=>$arItem)
{
	...
	/* 
		удалим значения свойств перед выводим.
		их добавили в ini,php - BeforeIndex
	*/
	$patterns[] = "/#(.)+#/i";
	$replacements[] = "";
	$arResult["SEARCH"][$i]["NAME"] = preg_replace($patterns, $replacements, $arItem["NAME"]);
